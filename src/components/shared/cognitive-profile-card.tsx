...
export function CognitiveProfileCard(props: CognitiveProfileCardProps) {
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-white/8 bg-white dark:bg-[#17130E] p-5">
        ...
        {metrics.map(({ key, label, sublabel, barClass, textClass, bgClass }) => {
          ...
          return (
            <div key={key} className="space-y-1.5">
              ...
              <Progress value={value ?? 0} className={`h-1.5 bg-slate-200 dark:bg-[#1C1710] ${barClass}`} />
            </div>
          );
        })}
      </div>
