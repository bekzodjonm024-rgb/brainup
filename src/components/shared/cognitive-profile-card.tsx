...
export function CognitiveProfileCard(props: CognitiveProfileCardProps) {
  return (
    <div className="rounded-2xl border border-stone-200 dark:border-white/8 bg-white dark:bg-[#17130E] p-5">
      <div className="flex items-center gap-2 mb-4">
        <div className="w-7 h-7 rounded-lg bg-violet-500/10 border border-violet-500/20 flex items-center justify-center">
          <Brain className="h-3.5 w-3.5 text-violet-400" />
        </div>
        <div>
          <p className="text-sm font-semibold text-[#1C1208] dark:text-white">Kognitiv profil</p>
          <p className="text-[11px] text-stone-400 dark:text-slate-600">Klinik tashxis emas — o&apos;quv moslashuvi uchun</p>
        </div>
      </div>
            ...
            <div key={key} className="space-y-1.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-stone-400 dark:text-slate-600 hidden sm:block">{sublabel}</span>
                </div>
                <span className={`text-sm font-bold ${value !== null ? textClass : "text-slate-300 dark:text-slate-700"}`}>
                  {value !== null ? `${value}` : "—"}
                  {value !== null && <span className="text-xs font-normal text-stone-400 dark:text-slate-600">/100</span>}
                </span>
              </div>
              <Progress value={value ?? 0} className={`h-1.5 bg-stone-200 dark:bg-[#1C1710] ${barClass}`} />
            </div>
          );
        })}
      </div>
