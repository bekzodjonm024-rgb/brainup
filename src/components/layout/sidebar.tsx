...
export function Sidebar({ role, userName, avatarUrl, onClose, badges }: SidebarProps) {
  ...
  return (
    <aside className="flex h-screen w-64 flex-col bg-white dark:bg-[#141b2d]" style={{ borderRight: "1px solid rgba(0,0,0,0.07)" }}>

      {/* Logo */}
      <div className="flex h-16 items-center gap-2.5 px-5" style={{ borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
        <BrainUPLogo size="sm" href="/" />
        <span className="text-lg font-extrabold text-slate-900 dark:text-white tracking-tight">BrainUP</span>
      </div>
            ...
            <Link
              ...
              className={cn(
                ...
                active
                  ...
                  : "text-slate-600 dark:text-slate-400 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-950/20 dark:hover:text-blue-400"
              )}
            >
      ...
      {/* Footer */}
      <div className="px-3 py-3 space-y-0.5" style={{ borderTop: "1px solid rgba(0,0,0,0.07)" }}>
        <div className="flex items-center gap-2.5 rounded-xl px-3 py-2">
          <div className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-full overflow-hidden bg-blue-100 dark:bg-blue-950/30">
            {avatarUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={avatarUrl} alt={userName} className="h-full w-full object-cover" />
            ) : (
              <span className="text-xs font-bold text-blue-600 dark:text-blue-400">{initial}</span>
            )}
          </div>
