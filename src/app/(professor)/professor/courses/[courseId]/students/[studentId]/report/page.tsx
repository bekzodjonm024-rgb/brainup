...
function masteryColor(score: number | null) {
  if (score === null) return "bg-stone-100 dark:bg-[#1C1710] border border-stone-200 dark:border-white/10 text-stone-500";
  if (score >= 0.85) return "bg-emerald-500/10 border border-emerald-500/20 text-emerald-400";
  if (score >= 0.6) return "bg-[#FEF4E7]/10 border border-[#B45309]/20 text-amber-400";
  ...
  return (
    <div className="flex flex-col flex-1 overflow-auto bg-[#F8F5EF] dark:bg-[#100D09]">
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        {/* Student info */}
        <div className="rounded-xl border border-stone-200 dark:border-white/8 bg-white dark:bg-[#17130E] px-5 py-4 flex flex-wrap items-center gap-4">
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-[#1C1208] dark:text-slate-200">
          ...
          <div className="flex flex-wrap gap-2">
            {student.yearLevel && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-stone-100 dark:bg-[#1C1710] border border-stone-200 dark:border-white/10 text-stone-500 dark:text-slate-400">{student.yearLevel}-kurs</span>
            )}
            {student.groupName && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-stone-100 dark:bg-[#1C1710] border border-stone-200 dark:border-white/10 text-stone-500 dark:text-slate-400">{student.groupName}</span>
            )}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { icon: <BookOpen className="h-5 w-5 text-[#B45309] dark:text-amber-400" />, label: "Boshlangan mavzu", value: `${topicsStarted} / ${topics.length}`, iconBg: "bg-[#FEF4E7] dark:bg-amber-950/30" },
            ...
            <div key={s.label} className="rounded-xl border border-stone-200 dark:border-white/10 bg-white dark:bg-[#1C1710] p-4 shadow-sm">
              ...
              <p className="text-xs text-stone-500 dark:text-slate-400 mt-1.5 uppercase tracking-wide font-medium">{s.label}</p>
              {"sub" in s && s.sub && <p className="text-xs text-stone-400 dark:text-slate-500 mt-0.5">{s.sub}</p>}
            </div>
          ))}
        </div>
          ...
          <div className="lg:col-span-2">
            <div className="rounded-xl border border-stone-200 dark:border-white/8 bg-white dark:bg-[#17130E] overflow-hidden">
              <div className="px-5 py-4 border-b border-stone-200 dark:border-white/8">
                <h3 className="text-sm font-semibold text-stone-600 dark:text-slate-300">Mavzular bo&apos;yicha progress</h3>
                <div className="h-1.5 bg-stone-200 dark:bg-[#1C1710] rounded-full overflow-hidden mt-2">
                  <div
              ...
              <div className="divide-y divide-stone-200 dark:divide-slate-800/60">
                {topics.map((topic) => {
                  ...
                  return (
                    <div
                      key={topic.id}
                      className="flex items-center gap-3 text-sm px-5 py-2.5 hover:bg-stone-50 dark:hover:bg-[#2a2720]/30 transition-colors"
                    >
                      ...
                      {k ? (
                        <>
                          <span className="text-xs text-stone-400 dark:text-slate-600 hidden sm:block shrink-0">
                            {k.attempts} urinish
                          </span>
                          {k.lastPracticedAt && (
                            <span className="text-xs text-stone-400 dark:text-slate-600 hidden md:block shrink-0">
        ...
        <div className="card-lift rounded-xl border border-stone-200 dark:border-white/8 bg-white dark:bg-[#17130E] p-5">
          <h3 className="text-sm font-semibold text-stone-600 dark:text-slate-300 mb-1">
