      ...
      <main className="flex-1 p-6 space-y-6 max-w-3xl">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {[student.university.name, student.faculty.name, `${student.yearLevel}-kurs`, student.groupName].filter(Boolean).map((tag) => (
                  <span key={tag} className="text-xs px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-slate-400">{tag}</span>
                ))}
              </div>
            </div>
        ...
        {student.cognitiveProfile ? (
          ...
          <div className="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-5 flex items-center gap-4">
            ...
            <div>
              <p className="font-medium text-slate-200">Baholash hali bajarilmagan</p>
        ...
        {/* Profile settings */}
        <div>
          <h2 className="f-syne font-bold text-slate-300 mb-3">Sozlamalar</h2>
          <ProfileSettings
            role="STUDENT"
        ...
        {student.enrollments.map((enrollment) => {
          ...
          return (
            <div key={enrollment.courseId} className="rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
                  ...
                  return (
                    <div key={topic.id} className="space-y-1.5">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-slate-400 truncate flex-1 mr-2">{topic.title}</span>
                        {mastery > 0 ? (
                          <MasteryBadge score={mastery} />
                        ) : (
                          <span className="text-xs text-slate-700">Boshlanmagan</span>
                        )}
                      </div>
