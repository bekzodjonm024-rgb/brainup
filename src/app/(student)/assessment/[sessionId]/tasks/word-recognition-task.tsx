...
export function WordRecognitionTask({ item, onComplete, disabled }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 space-y-6">
      <p className="text-slate-400 text-sm text-center">{item.prompt}</p>
      ...
      {phase === "delay" && (
        <div className="py-12 text-center">
          <div className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-slate-800 border border-slate-700 mb-4">
      ...
      {phase === "recognition" && (
        <>
          ...
          <div className="grid grid-cols-2 gap-2 max-h-64 overflow-y-auto rounded-xl border border-slate-800 bg-slate-950 p-2">
            {allOptions.map((word) => (
              <button
                key={word}
