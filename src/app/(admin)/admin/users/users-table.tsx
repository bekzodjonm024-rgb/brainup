          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 dark:bg-[#17130E] border-b border-stone-200 dark:border-white/8">
                <th className="text-left px-4 py-3 font-medium text-stone-500 dark:text-slate-600 text-xs uppercase tracking-wide">Talaba</th>
                <th className="text-left px-4 py-3 font-medium text-stone-500 dark:text-slate-600 text-xs uppercase tracking-wide hidden sm:table-cell">Guruh</th>
                ...
                <th className="text-left px-4 py-3 font-medium text-stone-500 dark:text-slate-600 text-xs uppercase tracking-wide hidden md:table-cell">Urinishlar</th>
                <th className="text-left px-4 py-3 font-medium text-stone-500 dark:text-slate-600 text-xs uppercase tracking-wide hidden lg:table-cell">Ro&apos;yxatdan</th>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5">
                            <p className="font-medium text-stone-700 dark:text-slate-200 truncate max-w-[140px]">{name}</p>
                            {s?.cognitiveProfile && (
                          ...
                          <p className="text-xs text-stone-400 dark:text-slate-600 truncate max-w-[160px]">{user.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {s?.university?.shortName && (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-stone-100 dark:bg-[#1C1710] border border-stone-200 dark:border-white/10 text-stone-500 dark:text-slate-400">{s.university.shortName}</span>
                        )}
                        {s?.yearLevel && (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-stone-100 dark:bg-[#1C1710] border border-stone-200 dark:border-white/10 text-stone-500 dark:text-slate-400">{s.yearLevel}-kurs</span>
                        )}
                        {s?.groupName && (
                          <span className="text-xs text-stone-400 dark:text-slate-600">{s.groupName}</span>
                        )}
                      </div>
                    </td>
